...
def route_page(request):
    ...
    if not route:
        user_preferences = f"""
            Я хочу завтрак каждый день. Поездка на {days_count} дней. Я хочу побольше погулять и посмотреть достопримечательностей. Посмотреть на исскуство и послушать музыку.
            """
